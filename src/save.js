...
export default function save({ attributes }) {
    ...
    // Get products without tabs (when no tabs exist)
    const productsWithoutTabs = products.filter(p => !p.tabId || p.tabId === '');
    ...
    return (
        <div {...blockProps}>
            <div className="product-models-block">
                ...
                {/* Tab Content - only render if tabs exist */}
                {tabs.length > 0 ? (
                    tabs.map((tab, index) => (
                        <div
                            key={tab.id}
                    ...
                    // No tabs - show all products directly
                    <div className="products-container">
                        {productsWithoutTabs.length > 0 ? (
                            productsWithoutTabs.map((product) => (
                                <div
                                    key={product.id}
                                    className="product-item"
                                    id={product.customId || undefined}
